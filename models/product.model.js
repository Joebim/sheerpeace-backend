...
const Product = {
  ...
  searchProducts: async (filters) => {
    ...
    // 🔹 Category & Subcategory Filter (Using Names)
    if (filters.category) {
      const category = await db("categories")
        .whereILike("name", filters.category)
        .first();
      if (category) {
        query.whereRaw(`? = ANY(category_ids)`, [category.id]);
      }
    }

    if (filters.subcategory) {
      const subcategory = await db("subcategories")
        .whereILike("name", filters.subcategory)
        .first();
      if (subcategory) {
        query.whereRaw(`? = ANY(subcategory_ids)`, [subcategory.id]);
      }
    }

    // 🔹 Brand Filter (Using Name Instead of ID)
    if (filters.brand) {
      const brand = await db("brands")
        .whereILike("name", filters.brand)
        .first();
      if (brand) {
    ...
    // 🔹 Color & Size Filters (Using Names Instead of IDs)
    if (filters.color) {
      const color = await db("colors")
        .whereILike("name", filters.color)
        .first();
      if (color) {
        query.whereRaw(`? = ANY(color_ids)`, [color.id]);
      }
    }

    if (filters.size) {
      const size = await db("sizes").whereILike("label", filters.size).first();
      if (size) {
        query.whereRaw(`? = ANY(size_ids)`, [size.id]);
      }
    }
